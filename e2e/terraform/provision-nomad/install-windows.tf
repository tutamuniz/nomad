...
resource "null_resource" "install_nomad_configs_windows" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "powershell Move-Item -Force -Path C://tmp/${var.role}-consul.hcl C://opt/nomad.d/${var.role}-consul.hcl",
      "powershell Move-Item -Force -Path C://tmp/${var.role}-vault.hcl C://opt/nomad.d/${var.role}-vault.hcl",
