resource "null_resource" "install_nomad_binary_windows" {
  ...
  # TODO: TF has a regression in the Windows file upload introduced in
  # https://github.com/hashicorp/terraform/pull/28626. We're stuck on
  # an older version of TF until that's resolved. Error looks like:
  #
...
resource "null_resource" "install_consul_configs_windows" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "powershell Move-Item -Force -Path C://tmp/consul_ca.pem -Destination C://etc/consul.d/ca.pem",
      "powershell Move-Item -Force -Path C://tmp/consul_ca.pem C://etc/consul.d/ca.pem",
